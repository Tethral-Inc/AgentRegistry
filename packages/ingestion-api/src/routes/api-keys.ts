...
import { query, queryOne, execute, sha256, makeError, createLogger } from '@acr/shared';
 ...
 * Operators create keys to access paid-tier features
 * (full friction breakdown, component-level analysis).
 */
...
// GET /api/v1/api-keys?operator_id=xxx - List keys for an operator
app.get('/api-keys', async (c) => {
  ...
  const keys = await query<{
    ...
    `SELECT key_hash AS "key_hash", name AS "name", tier AS "tier",
     ...
     revoked AS "revoked"
     FROM api_keys WHERE operator_id = $1
...
// DELETE /api/v1/api-keys/:key_hash - Revoke a key
