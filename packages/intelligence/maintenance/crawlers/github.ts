...
export class GitHubCrawler implements CrawlSourceAdapter {
  ...
  async discover(): Promise<DiscoveredSkill[]> {
    ...
    for (let qi = 0; qi < SEARCH_QUERIES.length; qi++) {
      const searchQuery = SEARCH_QUERIES[qi]!;
      const maxPages = qi === 0 ? MAX_PAGES : 3;
      ...
      for (let page = 1; page <= maxPages; page++) {
        ...
        try {
          ...
          const url = `${this.baseUrl}/search/code?q=${q}&per_page=${PER_PAGE}&page=${page}`;
          ...
          if (res.status === 403 || res.status === 429) {
            log.warn({ status: res.status, query: searchQuery }, 'GitHub rate limited, stopping discovery');
            break;
          }

          if (!res.ok) {
            log.warn({ status: res.status, query: searchQuery }, 'GitHub search failed');
            break;
          }
          ...
          log.error({ err, page, query: searchQuery }, 'GitHub search page failed');
          break;
        }
      }
    }
