# MCP Spec Proposal: `priorityHint` Tool Annotation

**Repository:** [modelcontextprotocol/specification](https://github.com/modelcontextprotocol/specification)
...
The `_meta` field exists for vendor-specific extensions, and a server could use `_meta.priority` today. However:
...
This is a pragmatic stopgap. Hosts won't read it without custom logic, but it lets server authors declare intent now and migrate to the standardized field later. If this proposal is accepted, `_meta.priorityHint` becomes a deprecated alias.
